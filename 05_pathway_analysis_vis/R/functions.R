...
#' GSEA dot plot in up and down regulated pathways facetted by condition
#'
#' @param data
#' @param title
#'
#' @return plot
plot_top_GSEA <- function(data, title) {
  test <- data |>
    dplyr::group_by(comp) |>
  ...
  ggplot2::ggplot(data = test, ggplot2::aes(y = ID, x = direction, color = `p.adjust`, size = NES)) +
    ggplot2::geom_point() +
...
#'
#' @return
upset_plot_GSEA_dat_fun <- function(data) {
  comp_name <- unique(data$comp)
  out <- data |>
    dplyr::filter(p.adjust < 0.05) |>
    dplyr::select(ID) |>
    dplyr::rename(comp_name = ID) |>
    t() |>
    as.vector()
}
...
myTheme <- list(
  "bw" = theme(
    ...
    axis.text = ggplot2::element_text(size = 8, colour = "black", family = "sans"),
    axis.title = ggplot2::element_text(size = 9, colour = "black", family = "sans"),
    strip.text = ggplot2::element_text(size = 9, colour = "black", family = "sans", margin = ggplot2::margin(2,2,2,2,"pt")),
    legend.text = ggplot2::element_text(size = 8, colour = "black", family = "sans"),
    legend.title = ggplot2::element_text(size = 9, colour = "black", family = "sans"),
...
upset_gg <- function(setList, setOrder = NULL, intersectOrder = "level", color = "black"){
  ...
  iP <- ggplot2::ggplot(iPlot, ggplot2::aes(intersect, size))+
    ggplot2::geom_bar(stat = "identity", fill = color)+
    ggplot2::geom_text(ggplot2::aes(y = size * 1.1, label = size), size = 3, vjust = 0, color = color)+
